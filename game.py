...
from interface import InterfaceTools, InterfaceEmpty, InterfacePauseMenu
...
class Game:
	...
	def pause(self):
		self.isPaused = True
		self.currentInterface = InterfacePauseMenu()
		sound.stop("music0")

	def unpause(self):
		self.isPaused = False
		self.currentInterface = InterfaceEmpty()
		sound.play("music0", int(self.tick*1000))
	...
	def update(self, deltaTime):
		...
		if response != None:
			...
			if isinstance(self.currentInterface, InterfacePauseMenu):
				# Return to game
				if response == 0:
					self.unpause()
				elif response == 1:
